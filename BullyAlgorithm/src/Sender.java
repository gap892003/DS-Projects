	...
	public void run() {
		...
		try {
			...
			if (serverPeer != null){
				...
				sock = new Socket();
				sock.connect( new InetSocketAddress (request.ipAddressofReceiver, request.portNumberOfReceiver), TIME_OUT);
				System.out.println( "Sending request from" + Listener.getInstance().getIPAddress() + " "
						+ Listener.getInstance().getPortNumber()
			...
			System.out.println ( request.ipAddressofReceiver + " is down!" );
